...
public class ParserCalc {
    ...
    private void getToken() {
        ...
        if ('p' == (text.charAt(idText))) {
            ...
            idText++;
            tokenType = NUMBER;
        } else if (isDelimiter(text.charAt(idText))) {
            token += text.charAt(idText);
            idText++;
            tokenType = DELIMITER;
        } else if (isFunc(text.charAt(idText))) {
            idText++;
            tokenType = FUNC;
        } else if (Character.isDigit(text.charAt(idText))) {
    ...
    private double func() {
        ...
        while (text_f > 0 && text_f < 6) {
            ...
            switch (text_f) {
                case 1:
                    result = Math.sin(partialresult);
                    break;
                case 2:
                    result = Math.cos(partialresult);
                    break;
                case 4:
                    result = Math.tan(partialresult);
                    break;
                case 3:
                    result = Math.exp(partialresult);
                    break;
                case 5:
